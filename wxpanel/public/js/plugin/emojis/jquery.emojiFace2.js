	...
	c.fn.emojiInit2 = function(b) {
		...
		if (void 0 == b || 0 >= b.length) option.error(null, "target object is undefined");
			...
			option.faceList.forEach(function(b) {
				a += "<i onclick='insertAtCaret2(\"" + option.targetId + '","' + b + "\",this)' style='font: normal normal normal 14px/1 FontAwesome;cursor: pointer;padding:3px;font-size:" + option.fontSize + "px;width: 20px;display: inline-block;text-align:center;'>" + b + "</i>"
			});
			b.css("width", "100%");
			...
			b.after("<i id='faceEnter2' onclick='showFaceBlock2()' style='padding: 5px;position: absolute;right: 19px;top: 4px;cursor: pointer;font: normal normal normal 14px/1 FontAwesome;'>😃</i>");
			b.after("<div id='faceBlock2' style='background:rgb(216, 216, 216);border-radius: 12px;display:none;position: absolute;border: 1px solid #e2e2e2;padding: 5px;right: -150px;top: 25px;width: 300px;'>" + a + "</div>");
			c(document).click(function() {
				c("#faceBlock2").hide()
			});
			c("#faceBlock2").click(function(a) {
				a.stopPropagation()
			});
			c("#faceEnter2").click(function(a) {
				a.stopPropagation()
			})
		}
	}
})(jQuery);
function showFaceBlock2() {
	$("#faceBlock2").show();
}
function insertAtCaret2(c, d, b) {
	try {
		$("#faceBlock2").hide();
		var a = $("#" + c).get(0);
