        ...
        var resourceTable = $('#' + resourceTableId).dataTable({
           ...
           "oTableTools" : {
            ...
            "initComplete": function ()
            {
				...
				//filtro Seccion
				var $select = $('<select id="secciones" />').css('width','160px');
				var options = JSON.parse('{!! $aViewData['secciones']->toJson() !!}');
				$('.filtro-custom input').attr('placeholder', 'Buscar...');
				
				if (options.length) {
					$.each(options, function() {
						$select.append($("<option />").val(this.id_seccion).text(this.seccion));
					});
				}
				$select.prepend($('<option selected value="" />').text('Todas las secciones...'))
            ...
            "aoColumnDefs": [
                { "mData": "icono", "aTargets":[0], "sortable":false, "sClass": "center", "mRender": function(value, type, full){
                    var icono = '';
                    if(full.icono){
                        icono = '<i class="fa fa-2x '+full.icono+'"></i>';
                    }
                    return icono;
                }
                },
                { "mData": "titulo", "aTargets":[1], "sortable":true },
				{ "mData": "seccion", "aTargets":[2], "sortable":false },
				{ "mData": "sumario", "aTargets":[3], "sortable":true },
				{ "mData": "orden", "aTargets":[4], "sortable":true },
