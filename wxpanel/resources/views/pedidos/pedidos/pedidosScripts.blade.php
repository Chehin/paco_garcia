<script type="text/javascript">
    ...
    $(document).ready(function() {
		...
		resourceReq.notificaciones.url = function(id){
            return appCustom.{{$aViewData['resource']}}.notificaciones.EDIT.url(id);
        };
        ...
        var resourceTable = $('#' + resourceTableId).dataTable({
            ...
            "initComplete": function ()
            {
				...
				var select = '{{ Form::select(
					$selectName, 
					[	
						"cliente"          => \config('appCustom.clientName'),
						"MercadoE"          => "Mercado Envios",
						"Andreani"          => "Andreani",
						"Mis envios"          => "Mis envios",
					],
					null,
					['style' => 'width:124px;']
					) 
				...
				$(dtWrapper + '.filtro-mas').append(select);
				$(dtWrapper + 'select[name='+ selectName +']').prepend('<option selected value="0">Met Envio(Todos)</option>');
				$(dtWrapper + '.filtro-mas select[name='+ selectName +']').on('change', onChangeFiltroMas8);
				...
				var select = '{{ Form::select(
					...
					[
						"0"          => "Pedidos Web",
						"1"          => "Pedidos Mercado Libre",
					],
					0,
					['style' => 'width:auto;', 'class' => 'filtro_ped_meli']
					) 
				}}';
							
				$(dtWrapper + '.filtro-meli').append(select);
				$(dtWrapper + 'select[name='+ selectName +']');
				$(dtWrapper + '.filtro-meli select[name='+ selectName +']').on('change', onChangeFiltroMas9);
				
			*/
				$(dtWrapper + '.filtro-mas').css('top', '-69px');
            ...
            "aoColumnDefs": [
				...
				{ "mData": "notificaciones", "aTargets":[14], "sortable":false, "mRender": 
					function(value, type, full){
						var notificaciones='<a href="javascript:void(0);" ><span data-href="'+resourceReq.notificaciones.url(full.id_pedido)+'" data-toggle="modal-custom" class="fa fa-info-circle"></span></a> ';
	...
	var onChangeFiltroMas9 = function(){
			$('#' + resourceTableId).dataTable().fnFilter($(this).val() ,9);
		}
