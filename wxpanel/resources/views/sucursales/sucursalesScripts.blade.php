<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        resourceReq.index.url = appCustom.sucursales.INDEX.url;
        resourceReq.index.verb = appCustom.sucursales.INDEX.verb;
        
        resourceReq.create.url = appCustom.sucursales.CREATE.url;
        ...
        resourceReq.edit.url = function(id){
            return appCustom.sucursales.EDIT.url(id);
        };
        resourceReq.edit.verb = appCustom.sucursales.EDIT.verb;
        ...
        resourceReq.delete.url = function(id){
            return appCustom.sucursales.DELETE.url(id);
        };
        resourceReq.delete.verb = appCustom.sucursales.DELETE.verb;
        
        resourceReq.language.url = function(id){
            return appCustom.sucursales.language.mainView.url(id)
			//return appCustom.{{$aViewData['resource']}}.language.mainView.url(id)
        };
		
		resourceReqOption1.edit.url = function(id){
            return appCustom.sucursales.image.mainView.url(id);
        };
		
		resourceReqOption2.edit.url = function(id){
            return appCustom.sucursales.noteRelated.mainView.url(id);
        };
        ...
        var resourceTable = $('#' + resourceTableId).dataTable({
           ...
           "oTableTools" : {
            ...
            "aoColumnDefs": [
                { "mData": "antetitulo", "aTargets":[0], "sortable":true },
                { "mData": "titulo", "aTargets":[1], "sortable":true },
				{ "mData": "orden", "aTargets":[2], "sortable":true },
                ...
                { "mData": "", "aTargets":[5], "sortable":false, "mRender": function(value, type, full){
                                        var prefix = '2';
                                        var resourceTableId = prefix + '_' + resourceTableId;
                                        var checked = (1 == full.sucursalEnvio) ? 'checked' : '';

                                        var favoriteOption = '<div class="btn-group" style="margin-right:30px">'+
                                                                '<span class="onoffswitch" name="enable2" title="Destacar / No Destacar">'+
                                                                        '<input type="checkbox" name="'+ resourceTableId + '_activeItem_'+full.id+'" class="onoffswitch-checkbox" value="'+full.id+'" '+ checked +' id="'+ resourceTableId + '_activeItem_'+full.id+'">'+
                                                                        '<label class="onoffswitch-label" for="'+ resourceTableId + '_activeItem_'+full.id+'"> '+
                                                                                '<span class="onoffswitch-inner" data-swchon-text="SI" data-swchoff-text="NO"></span>'+ 
                                                                                '<span class="onoffswitch-switch"></span>' +
